...
-(void)sendIRNRequest:(NSMutableDictionary*)orderDetails withResult:(IRNResult)result{
    ...
    for (NSString *key in sortedKeys) {
        ...
        if ([encodedKey isEqualToString:@"MERCHANT"]
            ...
            || [encodedKey isEqualToString:@"IRN_DATE"]) {
            continue;
        }
    ...
    [hashs insertObject:[NSString stringWithFormat:@"%d%@",[[orderDetails valueForKey:@"IRN_DATE"] length],[orderDetails valueForKey:@"IRN_DATE"]] atIndex:4];
    ...
    NSMutableURLRequest *request = [[NSMutableURLRequest alloc] initWithURL:[NSURL URLWithString:@"https://secure.ypmn.ru/order/irn.php"] cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:30.0f];
